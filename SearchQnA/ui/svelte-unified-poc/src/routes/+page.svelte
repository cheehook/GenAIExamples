...
<script lang="ts">
	...
	import ChatInput from "$lib/shared/components/dummy_components/ChatInput.svelte";
	...
	let components: Component[]  = data.components? data.components : [];
	...
	const handleSubmit = (value, id) => {
		console.log(`Submitted: ${value} for component ID: ${id}`);
	};
			...
			{#if components?.length}
				{#each components as component (component.component_id)}
					<ChatInput
						...
						{loading}
						onSubmit={(value) => handleSubmit(value, component.component_id)}
					/>
				{/each}
			{:else}
